...
import { chatProvider, chatProviderConfig } from "./agent/providers";
import { CHAT_LITE_MODEL } from "./agent/models";
import { resolvePaper } from "./paperIngest/resolve";
import { normalizeDoi } from "./paperIngest/identifiers";
import {
  classifyPaperText,
  isUsablePaperMetadata,
  normalizeLlmPaperMetadata,
} from "./paperIngest/uploadIdentifiers";
    ...
    if (classified) {
      ...
      if (
        ...
        const written = await ctx.runMutation(
          internal.paperExtractions.upsertResolvedPaperMetadata,
          {
...
async function markScholarly(
  ...
  await ctx.runMutation(internal.paperExtractions.setArtifactDetectedKind, {
    ownerUserId: args.ownerUserId,
    artifactId: args.artifactId,
