...
import type { FeedConsensus, FeedConsensusPaper } from "./feedModel";
...
export const getConsensus = action({
  ...
  > => {
    ...
    const cached = await ctx.runQuery(internal.feedConsensus.getConsensusCache, {
      questionKey,
    });
    ...
    await ctx.runMutation(internal.feedConsensus.putConsensusCache, {
      questionKey,
      question,
      yes,
      no,
