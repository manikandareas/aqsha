import { fetchWithTimeout, userAgent } from "./ingest/http";
...
// Lowercased prefixes that mark non-article cruft (share bars, related rails,
// ad slots) on Indonesian fact-check sites. Conservative on purpose.
...
const META_IMAGE_PATTERNS: RegExp[] = [
  ...
  // Some templates emit `content` before `property`/`name`.
...
export async function fetchArticlePreview(
  ...
  // The factcheck-explorer fallback URL has no review page to read.
...
export function extractArticleTextFromHtml(html: string): string | undefined {
  ...
  const cleaned = scope
    ...
    .replace(
      /<(nav|header|footer|aside|form|figure|figcaption)\b[^>]*>[\s\S]*?<\/\1>/gi,
      " ",
    );
...
// Clean already-extracted plain text (e.g. Exa's markdown-ish article text),
// applying the same paragraph filter as the HTML path so both sources read the
// same. Strips leading markdown heading/bullet markers per line.
...
function isContentParagraph(text: string): boolean {
  ...
  // Real prose carries sentence punctuation; nav menus / link-lists (common in
  // news-site bodies) do not, so this drops them without a per-site selector.
