...
export type ThreadResearchSource = ResearchSourceLike & {
	turnId: string;
...
export function messageSourceCards(
	...
	turnSources: readonly ThreadResearchSource[] | undefined
): SourceCardData[] {
...
export function buildThreadPanelLookups(
	...
	researchSources: readonly ThreadResearchSource[] | undefined,
	planGate: MastraPlanGate | null,
	askGate: MastraAskGate | null,
	statsGroups: readonly ThreadStatsGroup[] | undefined
): ThreadPanelLookups {
	const sourcesByTurn = new Map<string, ThreadResearchSource[]>();
	...
	const bySubQ = new Map<string, ThreadResearchSource[]>();
	for (const s of researchSources ?? []) {
