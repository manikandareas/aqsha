...
export type MastraTimelineOptions = {
	/** Project-document clients opt in; other clients keep the generic tool row. */
...
function messageCreatedAt(m: MastraDBMessageLike): number {
	...
	const signal = m.content?.metadata?.signal as
		{ acceptedAt?: unknown; createdAt?: unknown } | undefined;
 ...
 */
export function mastraMessagesToTimeline(
	messages: readonly MastraDBMessageLike[],
	options: MastraTimelineOptions = {}
): TimelineMessage[] {
	return messages.map((m) => {
		...
		raw.forEach((p, i) => {
			...
			if (type === 'text') {
				...
				if (text.trim())
					parts.push({ kind: 'reasoning', id: `${m.id}:r${i}`, text, thinking: false });
				...
				if (artifact) {
					...
					const proposal =
						options.documentProposals && inv.state === 'result'
							? documentProposalFromResult(toolName, inv.result)
							: null;
					if (proposal) {
						parts.push({
...
export function settleAssistantTurn(state: MastraTimelineState): MastraTimelineState {
	...
	const messages =
		idx < 0
			? state.messages
			: state.messages.map((m, i) => (i === idx ? { ...m, streaming: false } : m));
	// HITL menggantung (approval / ask-gate) → pertahankan status (turn menunggu user), jangan "ready".
	const status: MastraStatus = state.approvals.length > 0 || state.askGate ? state.status : 'ready';
...
function lastAssistantHasAnswerText(state: MastraTimelineState): boolean {
	for (let index = state.messages.length - 1; index >= 0; index -= 1) {
		...
		return message.parts.some((part) => part.kind === 'text' && part.text.trim().length > 0);
	}
	return false;
}
...
export function reduceMastraChunk(
	...
	switch (chunk.type) {
		...
		case 'tool-output': {
			...
			const proposal =
				!isError && options.documentProposals
					? documentProposalFromResult(toolName, result)
					: null;
 ...
 */
export function reviveWorkflowTurn(state: MastraTimelineState, runId: string): MastraTimelineState {
	return {
		...state,
		status: 'streaming',
		runId,
...
export function seedWorkflowProgress(
	...
	if (existing >= 0) {
		...
		next = {
			...seeded,
			messages: seeded.messages.map((m, i) => (i === idx ? { ...m, turnId: runId } : m))
		};
	}
	for (const stepId of WF_STEP_ORDER) {
		...
		if (!status) continue;
		next = upsertWorkflowStep(
			next,
			idx,
			stepId,
			wfStepStatusToTool(status),
			num(sr?.startedAt) || undefined
		);
...
export function reduceWorkflowChunk(
	...
	switch (chunk.type) {
		case 'workflow-start': {
			...
			return {
				...s,
				messages: s.messages.map((m, i) => (i === idx ? { ...m, turnId: runId } : m))
			};
		}
...
function setDeepReasoning(
	...
	return upsertPart(
		state,
		msgIdx,
		{ kind: 'reasoning', id: 'wf:reasoning', text, thinking: false },
		false
	);
}
...
function setStepDetailWith(
	...
	return mutateMessage(state, msgIdx, (parts) => {
		...
		if (i < 0) {
			...
			return [
				...parts,
				{ kind: 'tool', id: `tool:${toolCallId}`, model: buildStepModel(stepId, 'running', detail) }
			];
		}
		return parts.map((p, j) => {
...
function deepProcessParts(deepProcess: Record<string, unknown>): TimelinePart[] {
	...
	const push = (stepId: string, detail: DeepStepDetail) => {
		parts.push({
			kind: 'tool',
			id: `tool:wf:${stepId}`,
			model: buildStepModel(stepId, 'completed', detail)
		});
	};
	...
	if (vizIds.length > 0 || analyzedCount > 0) {
		push('analyze-sources', {
			kind: 'text',
			text: formatDeepAnalyzeSummary(analyzedCount, vizIds)
		});
	}
...
function upsertPart(
	...
	return mutateMessage(state, msgIdx, (parts) => {
		...
		if (existing >= 0)
			return skipIfExists ? parts : parts.map((p, i) => (i === existing ? part : p));
		return [...parts, part];
	});
}
...
function appendToolArgsDelta(
	...
	return mutateMessage(state, msgIdx, (parts) => {
		...
		if (i < 0) {
			...
			return [
				...parts,
				{ kind: 'tool', id: `tool:${toolCallId}`, model: { ...model, argsTextRaw: delta } }
			];
		}
		return parts.map((p, j) => {
			if (j !== i || p.kind !== 'tool') return p;
...
function replaceWithArtifact(
	...
	return mutateMessage(state, msgIdx, (parts) => {
		...
		return [
			...without,
			{ kind: 'artifact', id: `artifact:${artifact.toolCallId}`, model: artifact }
		];
	});
}
...
function mutateMessage(
	...
	const messages = state.messages.map((m, i) => (i === msgIdx ? { ...m, parts: fn(m.parts) } : m));
