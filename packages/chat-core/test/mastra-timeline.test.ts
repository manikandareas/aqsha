...
} from "../src/timeline/mastra";
import type { TimelineMessage, TimelinePart } from "../src/timeline/types";

const projectTimelineOptions = { documentProposals: true } as const;
const mastraMessagesToTimeline = (
  messages: Parameters<typeof mapMessages>[0],
) => mapMessages(messages, projectTimelineOptions);
const reduceMastraChunk = (
...
describe("initial + optimistic turn", () => {
  ...
  it("carries attachmentIds onto the user bubble when provided", () => {
    const s = startAssistantTurn(initialMastraTimeline(), "lihat file", "t2", [
      "a1",
      "a2",
    ]);
...
describe("tool parts", () => {
  ...
  it("keeps an error document-edit result as an ordinary failed tool row", () => {
    ...
    expect(
      lastAssistant(s).parts.some((p) => p.kind === "document-proposal"),
    ).toBe(false);
    expect(lastAssistant(s).parts).toContainEqual(
  ...
  it("accumulates tool-call-delta args and renders input rows progressively", () => {
    ...
    if (tool?.kind === "tool") {
      expect(tool.model.rows.find((r) => r.key === "query")?.value).toBe(
        "academia",
      );
    }
  });
  ...
  it("writes the synthesize report as the answer text", () => {
    ...
    s = reduceWorkflowChunk(
      ...
      chunk(
        ...
        { id: "synthesize", output: { report: "# Laporan" } },
        "run-1",
      ),
    );
...
describe("seedWorkflowProgress (refresh re-attach)", () => {
  it("rebuilds the stepper + report from a runById snapshot, idempotently", () => {
    ...
    const steps = {
      "draft-plan": {
        status: "success",
        output: { plan: "p", subQuestions: ["x"] },
      },
      "search-literature": {
        status: "success",
        output: { subQuestions: ["x"] },
      },
    ...
    expect(
      lastAssistant(s2).parts.filter((p) => p.kind === "tool"),
...
describe("mastraMessagesToTimeline (rehydrate)", () => {
  ...
  it("keeps an unfinished document-edit invocation with forged result as an ordinary tool row", () => {
    ...
    expect(out[0].parts.some((p) => p.kind === "document-proposal")).toBe(
      false,
    );
    expect(out[0].parts).toContainEqual(
