...
/**
 * Single subprocess boundary untuk compile/ekspor dokumen (typst, pandoc), dipakai dua runtime:
 * Bun (API + worker) dan Node (runtime agen Mastra di-bundle ke Node — `globalThis.Bun` tak ada
 * di sana, sehingga spawn lewat Bun melempar "Cannot read properties of undefined"). Pilih
 * `Bun.spawn` bila global Bun ada, kalau tidak jatuh ke `node:child_process` dengan kontrak
 * stream/timeout/kill yang sama. apps/web ikut men-typecheck file ini saat menurunkan tipe Eden
 * `App` dan tak memuat bun-types — jadi kedua runtime dicapai lewat surface bertipe lokal, bukan
 * global ambient, agar type-graph web tetap bebas dari global `Bun`.
 */
type SpawnedProcess = {
