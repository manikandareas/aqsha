...
- **Prod (P10):** `eve build` lalu `eve start` sebagai proses managed **single replica** (volume `.workflow-data` + backup); web-v2 proxy via `withEve` (`EVE_NEXT_PRODUCTION_ORIGIN`). Nginx stream-safe di `app.aqsha.app` (`proxy_buffering off`, `read_timeout 3600s`, `http_version 1.1`, `Connection ''`).
