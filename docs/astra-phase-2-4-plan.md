...
### Phase 3 progress (2026-06-12 — blocker-resolution roadmap S1–S8 largely executed, 236 → 305 tests; commits `bc6d074`, `ee18baa` on `development`)

Each increment gate-green (typecheck · lint · convex test) + live-validated on dev. Full detail: `[[astra-phase3-implementation]]`. The R0–R4 execution status lives in `docs/astra-blocker-resolution-plan.md` §"Execution status".

**Executed (this roadmap):** R0 dev-run smoke harness (`agent/evals/devHarness.ts` + `scripts/smoke-deep-research.mjs`); R1a `runResearchRound`/`runCounterEvidence`/`finalizeRoundEvidence` extraction; R1b `literatureRoundAgent` + `counterEvidenceAgent` + `loadRunEvidence` + the `DEEP_RESEARCH_V2` workflow branch (flag read in a step — the workflow runtime has no `process`); R1c `citationAgent` + `writerAgent` (shared `generateSynthesis`); R2/AUD-08 RAG-on-resume via `contextHandler`; R3.1/AUD-17 prompt↔RAG dedup; R4 writer/citation skill delegation; R1d verification-incomplete report caveat + non-throwing between-round cancel check. **v1 + v2 happy smoke both pass live** (4 rounds → artifact → 13–16 citation checks → verification=revised).
...
- **3.1 Decompose** ✅ **v2 decomposed path live-validated behind `DEEP_RESEARCH_V2`** (default off; `bc6d074`). `literatureRoundAgent` + `counterEvidenceAgent` run as workflow steps over the shared `runResearchRound`/`runCounterEvidence`; `loadRunEvidence` rebuilds synthesis inputs from rows; `citationAgent` (pre-writer) + `writerAgent` (stages `draftMarkdown`) replace the monolith tail; per-subagent budget envelope folded via `updateBudget`; non-throwing between-round cancel check. **Remaining (Daytona-gated, deferred):** the `ensureRunArtifact`-shell + `finalizeArtifactContent` split that lets `statisticalAgent` attach computations *before* the auditor — on dev Daytona is unavailable so stat degrades regardless; v2 keeps the post-persist service-mode stat pass + a verification-incomplete report caveat. v1 monolith (`researchLoop`/`synthesize`/`auditClaims`) intentionally retained until the stabilization window (see blocker doc).
- **3.4 Domain skills + AUD debt** ✅ **AUD-08 + AUD-17 + delegation done.** Prior: 3 domain SKILL.md packs, `skillUpload.ts`, AUD-15/07/16. New: **AUD-08** RAG-on-resume (`contextHandler` + `agentRuns.visiblePromptSnapshot`); **AUD-17** prompt↔RAG dedup (`buildPromptContextForThread` → `{block, includedArtifactIds}` → `excludeArtifactIds` filter); **per-subagent skill delegation** (writer resolves its domain pack via `selectDomainPack`, citation records `citation-verification`; recorded in `skillActivations` with the step key).
...
1. **✅ RESOLVED — 3.1 execution rewrite live-validated.** The "can't be driven headlessly" framing was dissolved by the R0 dev harness (`npx convex run` against the internal seam): the auth'd-thread → deep-message → approval → multi-minute-workflow cycle is now a scripted, repeatable smoke gate (`scripts/smoke-deep-research.mjs`). v1 + v2 happy both pass live behind the `DEEP_RESEARCH_V2` strangler flag. (Diagnosed + fixed along the way: a `process`-in-workflow-runtime `ReferenceError` and a plan-approval transient-completed flip the harness must poll through.)
...
3. **✅ RESOLVED — AUD-08 RAG-on-resume.** Implemented via `@convex-dev/agent`'s `contextHandler` (never persisted), not `prompt` (which would clobber the HITL resolution message at `promptMessageId`). `startInlineRun` stashes `agentRuns.visiblePromptSnapshot` + attachment ids; `resumeGeneration` rebuilds the RAG block and injects it through a pure handler that reproduces the agent's default message ordering with one synthetic system message before the resolution turn. Unit-tested (`resumeContext.test.ts`, `resumeRecall.test.ts`).
4. **✅ RESOLVED — AUD-17 prompt↔RAG dedup.** `buildPromptContextForThread` now returns `{block, includedArtifactIds}` (an artifact is "included" only when neither the per-artifact 4k clip nor the 16k budget dropped content); threaded `scheduleGenerationForMessage → startInlineRun → generateReply → runInlineGeneration → buildRagContextForThread` `excludeArtifactIds` filter (compiler-checked sites). The HITL resume rebuild excludes the run's stored included ids too. Unit-tested (`contextDedup.test.ts`). **Remaining polish:** the R3.2 single-budget `assembleContext` unification (per-kind caps + clip report) — the behavioral dedup is done; the constants unification is a follow-up.
