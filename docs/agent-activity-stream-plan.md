...
### Perbaikan dari uji E2E live (2026-06-13)
- **Bentuk `tool_response` (gotcha SDK):** terverifikasi live, PostToolUse mengirim **array content telanjang** `[{type:"text",text:JSON}]`, BUKAN `{content:[…]}` yang diasumsikan decoder. Akibatnya `resultSummary` kosong → tak ada `· N hasil`. **Fix:** `responseContentArray()` menerima KEDUA bentuk (array langsung + `.content`); test bentuk-live ditambahkan. (Ini persis temuan dismissed "shape unverified" yang ternyata nyata — sanitizer defensif mencegah crash/leak, tapi enrichment-nya gagal sampai shape diperbaiki.)
- **Bug streaming follow-up (pra-ada, di luar activity-stream):** `useSmoothText` dipanggil SEKALI di level `ChatThreadState` dan dipakai ulang untuk SEMUA pesan asisten. Hook ini append-only (cursor hanya maju, tak pernah reset saat `text` berganti/menyusut — lihat impl `@convex-dev/agent`). Saat follow-up, cursor + visibleText giliran sebelumnya bocor ke gelembung baru sampai stream baru melewati panjang lama → "respons lama muncul sekejap lalu diganti". **Fix:** pindahkan smoothing ke komponen `StreamingResponse` di dalam `MessageRow` (transcript meng-key tiap baris per message id → tiap giliran remount hook → cursor fresh).
