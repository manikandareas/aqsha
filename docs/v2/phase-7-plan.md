...
### Catatan progress (2026-06-22 — 7.2+7.3 as-built)

7.2 + 7.3 selesai sekaligus (branch sama, gates hijau, ZERO migration). Highlight + deviasi:

- **CitationService** (`research/citation.ts`) + **similarity.ts** (port pure 6.4-deferred) re-export dari subpath `@aqsha/services/research` yang sudah ada. **Adaptasi V2**: provider balas `[]` saat gagal *dan* nihil (tanpa failure-sentinel V1) → existence kosong = ambigu → `null` (unverifiable), **tak pernah** false `not_found`; `not_found` hanya bila hasil nyata datang & tak ada yang cocok. `chargeExternalSearch` → digeneralisasi jadi `chargeToolUsage({feature,...})`, wrapper tipis dipertahankan. citation_verify=0 kredit (usage-tracked).
- **FLAG-2 RESOLVED = opsi C (termurah+benar)**: eve mengekspos lineage induk di projeksi tool-facing `ctx.session.parent?.rootSessionId` (denormalisasi puncak rantai dispatch — terverifikasi di `eve/dist/.../callback-context.d.ts` + `channel/types.d.ts`). `threadScopeId(ctx) = ctx.session.parent?.rootSessionId ?? ctx.session.id` dipakai di `persistResearch` + `search_thread_documents` → sumber/RAG subagent ter-scope ke thread induk. Billing idempotency tetap child session. TAK perlu plumbing message/input (opsi A/B ditolak). **Runtime propagasi auth ke declared subagent tetap item manual-E2E.**
- **Plan card editable = deny-with-revision** (bukan approve-with-edited-input — eve TAK punya primitive itu; approve selalu jalankan `execute` dengan input model asli; user-text hanya sampai ke model lewat **deny** `execution-denied{reason}`). Setuju-apa-adanya = 1 klik; sunting→"Kirim revisi" = `deny`+JSON, SKILL usulkan-ulang. `once()` tetap gate re-approval (deny ≠ approved).
...
- **Subagent child-session scoping** — ✅ **RESOLVED (7.3 as-built)**: eve ekspos `ctx.session.parent.rootSessionId` di projeksi tool → `threadScopeId()` scope sumber/RAG ke thread induk. Sisa: verifikasi runtime propagasi auth/principal ke declared subagent (manual-E2E).
