...
- _Exit_: checklist Phase 0 asli (§6) tercentang dengan SDK sungguhan; anomali tercatat di 9.2 #0.
...
- `packages/convex/convex/schema.ts`: tambah `chatThreads`, `chatMessages`, `agentRuns2` _(atau nama baru bila bentrok dengan tabel lama)_, `agentRunEvents2`, `pendingInteractions` persis §4.5 + index per pola baca (by_thread, by_owner_activity, by_run_seq, by_thread_status).
...
- _Exit_: Phase 1.1 + 1.4 selesai; vitest convex-test untuk endpoint service; `convex dev --once` bersih.

**Step 2 — Integrasi `apps/web` (data-hooks + flag) (2–3 hari) — _milestone "bisa dites via UI"_** ✅ **SELESAI (2026-06-12) — dikonfirmasi owner via dogfood UI**
...
- _Exit (kriteria "selesai" versi owner)_: **dengan flag `sdk`, ketiga surface chat bisa dipakai dari UI**: kirim pesan → streaming balasan; askUser → kartu → jawab → lanjut; proposeArtifact → approve → artifact tertulis ke workspace; `/deep` jalan dengan run-progress; `/verify-citations` jalan. Dogfood internal dimulai di sini.
...
- _Exit_: Phase 2 exit criteria asli (§6) terpenuhi end-to-end. ✅
...
- _Exit_: Orkestrasi multi-fase resumable §5.5 terpenuhi; paritas kualitas formal menunggu eval di atas.
...
- _Exit_: `verifyStatistics` nyata ✅; biaya run tercatat akurat ✅.

**Step 6 — Dual-run → cutover → bersih-bersih** ✅ **SELESAI (2026-06-13)** — owner setuju cutover penuh (termasuk langkah tak-terbalikkan) + sudah dogfood Step 3–5.
...
- **Ladder eksekusi**: batch reversibel (kode, `git revert`-able) DULU, lalu DUA langkah destruktif tak-terbalikkan (unmount `app.use(agent)` = hapus permanen data thread lama per D7; drop tabel legacy) sebagai langkah ber-sign-off terpisah.
  ...
  - ✅ **6c (commit `4988802`, frontend sdk-only)**: twin v2 `listThreadsByWorkspace`, drop `addThreadContextArtifacts` (mati), rute SEMUA entry-point buat-thread (workspace-detail via pola dispatcher explore, discovery, settings) ke v2, buang legacy data-hook + flag `agent-backend` + `useUIMessages` + blok artifact per-pesan + legacy hitlResume + `optimistic-updates`. `useSmoothText` tetap dari `@convex-dev/agent/react` (dep npm tetap di web). typecheck+lint hijau.
  - ✅ **6d (commit `4bbd6ab`)**: hapus runtime legacy `convex/agent/` (astra/runtime/runLifecycle/messages/threadTitles/workflow + hitl/skills/evals/prompt(sebagian)/research(sebagian)/sandbox/tools + providers/domainReliability) + 37 file test legacy; `threads.ts` di-trim ke `assertThreadOwner`/`tryAssertThreadOwner` + `threadContext.getThreadOwner` di-rewire ke `chatThreads`; `artifacts/uploads.ts` buang `processPendingAttachmentsAndStart`. Convex 118 test hijau.
  - ✅ **6e (commit `c5cd75b`)**: unmount `app.use(workflow)` (tak dipakai lagi; tanpa data durable).
  - ✅ **6f (commit `40e594a`, TAK-TERBALIKKAN)**: unmount `app.use(agent)` (dev: "Unmounted component agent" — data thread legacy in-component terhapus permanen, D7); `accountCleanup` buang jalur `deleteAgentThreads` (referensi `components.agent` terakhir). **Drop tabel first-party legacy DITUNDA** (terdokumentasi): mayoritas masih dibaca kode yang bertahan — `agentRuns` (rekonsiliasi billing), `skillActivations` (skillRegistry), `messageWorkspaceActions` (workspaces), `threadMetadata`/`threadContext*`/`messageContext*` (RAG context-pinning) — jadi tak bisa di-drop tanpa refactor pembaca itu; yang benar-benar yatim butuh migrasi clear-data dulu sebelum drop skema bersih. Dibiarkan inert agar push tidak terblokir/ireversibel tanpa migrasi tertelaah. **[SELESAI 2026-06-13]** semua tabel legacy ini sudah di-drop via `docs/convex-legacy-cleanup-plan.md` (batch C1–C6): pembaca yang bertahan di-repoint (billing lepas `agentRuns`; `threadMetadata`→`chatThreads`; RAG ramping; skill subsystem & badge UI dibuang), baris dev dikosongkan sekali-pakai, lalu 21 def tabel + `const runId` + 3 kolom `v.id("agentRuns")` dihapus dari skema.
- _Exit_: Phase 4.3 — **SELESAI PENUH**: satu-satunya backend agent = `apps/agents` (Claude Agent SDK) di atas tabel first-party; komponen `@convex-dev/agent` + `@convex-dev/workflow` ter-unmount; runtime/FSM/skills-runtime legacy terhapus; `apps/web` sepenuhnya di jalur v2. **Utang drop tabel legacy: SELESAI** (lihat 6f).

**Status keseluruhan inisiatif: SELESAI** (Step 0–6). Pembersihan tabel legacy juga **SELESAI** (2026-06-13, `docs/convex-legacy-cleanup-plan.md`). Sisa = follow-up non-pemblokir yang tercatat di §9.3 (mis. OTEL collector deploy, eval paritas deep research, materialisasi skill user, billing reconciliation produk) + promosi cutover+cleanup ke produksi via `convex deploy` (dijalankan owner).
