...
- **Ladder eksekusi**: batch reversibel (kode, `git revert`-able) DULU, lalu DUA langkah destruktif tak-terbalikkan (unmount `app.use(agent)` = hapus permanen data thread lama per D7; drop tabel legacy) sebagai langkah ber-sign-off terpisah.
  ...
  - ✅ **6f (commit `40e594a`, TAK-TERBALIKKAN)**: unmount `app.use(agent)` (dev: "Unmounted component agent" — data thread legacy in-component terhapus permanen, D7); `accountCleanup` buang jalur `deleteAgentThreads` (referensi `components.agent` terakhir). **Drop tabel first-party legacy DITUNDA** (terdokumentasi): mayoritas masih dibaca kode yang bertahan — `agentRuns` (rekonsiliasi billing), `skillActivations` (skillRegistry), `messageWorkspaceActions` (workspaces), `threadMetadata`/`threadContext*`/`messageContext*` (RAG context-pinning) — jadi tak bisa di-drop tanpa refactor pembaca itu; yang benar-benar yatim butuh migrasi clear-data dulu sebelum drop skema bersih. Dibiarkan inert agar push tidak terblokir/ireversibel tanpa migrasi tertelaah.
- *Exit*: Phase 4.3 — **SELESAI PENUH**: satu-satunya backend agent = `apps/agents` (Claude Agent SDK) di atas tabel first-party; komponen `@convex-dev/agent` + `@convex-dev/workflow` ter-unmount; runtime/FSM/skills-runtime legacy terhapus; `apps/web` sepenuhnya di jalur v2. **Utang tersisa: data-clearing migration + drop tabel legacy yatim** (tidak memblokir; lihat 6f).

**Status keseluruhan inisiatif: SELESAI** (Step 0–6). Sisa = utang kebersihan (drop tabel legacy yatim setelah migrasi clear-data) + follow-up non-pemblokir yang tercatat di §9.3 (mis. OTEL collector deploy, eval paritas deep research, materialisasi skill user, billing reconciliation produk).
