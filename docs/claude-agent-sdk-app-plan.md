...
- *Exit*: Phase 4.3 — **SELESAI PENUH**: satu-satunya backend agent = `apps/agents` (Claude Agent SDK) di atas tabel first-party; komponen `@convex-dev/agent` + `@convex-dev/workflow` ter-unmount; runtime/FSM/skills-runtime legacy terhapus; `apps/web` sepenuhnya di jalur v2. **Utang drop tabel legacy: SELESAI** (lihat 6f).
