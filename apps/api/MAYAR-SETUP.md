...
Placeholder sudah disiapkan di **`apps/api/.env.local`** (Bun memuatnya menambah
`.env`) — tinggal isi nilainya. **Hanya api** yang butuh `MAYAR_*`; `agent`
...
Mayar (cloud) harus bisa menjangkau endpoint **publik** api. URL = path-secret:

```
https://<api-public-host>/webhooks/mayar/<MAYAR_WEBHOOK_SECRET>
```

- Dashboard: **Integration → Webhook** → isi URL di atas → Save → **Test URL**.
- Sandbox lokal: expose api via tunnel (ngrok/cloudflared) lalu pakai URL tunnel.
...
1. Redeploy **api** + **agent** (baca env baru).
