...
/**
 * Angka statis, tanpa count-up — untuk brand yang menjual traceability,
 * angka yang berubah-ubah saat scroll justru terbaca sebagai karangan.
 * "Semua" (frasa mekanisme) menggantikan klaim absolut "100%".
 */
const stats = [
  {
    display: "40%",
  ...
  {
    display: "Semua",
    label: "sumber yang masuk Aqsha dicek ke paper aslinya",
    footnote: false,
  },
  {
    display: "6",
...
function PinnedQuote({ quote }: { quote: (typeof quotes)[number] }) {
  ...
  return (
    <m.blockquote
      className="relative border-2 border-border bg-background px-7 py-8 sm:px-9 sm:py-10"
      initial={
        reduce
          ? false
    ...
    >
      ...
      <span
        aria-hidden
        className="absolute -top-3 left-1/2 h-6 w-20 -translate-x-1/2 rotate-[-2deg] border-2 border-foreground/10 bg-foreground/10"
      />
      <p className="font-heading text-pretty text-xl font-normal leading-[1.35] tracking-normal text-foreground sm:text-2xl sm:leading-[1.3]">
 ...
 * strip of giant static numerals (footnoted to the data source), two user complaints taped on as tilted
      ...
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-6 lg:grid-cols-[7fr_5fr] lg:items-end lg:gap-x-20">
          <div>
            ...
            <m.h2
              className="font-heading mt-3 max-w-[min(100%,38rem)] text-[2.5rem] font-medium leading-[1.08] tracking-normal text-foreground sm:mt-4 sm:text-[2.75rem] sm:leading-[1.06] lg:text-[3.25rem] lg:leading-[1.05]"
        ...
        >
          {stats.map((stat) => (
            <div key={stat.label}>
              <span className="font-heading block text-6xl font-normal leading-none tabular-nums text-foreground sm:text-7xl lg:text-8xl">
                {stat.display}
                {stat.footnote && (
                  <a
                    href="#sumber-data"
                    className="ml-1.5 align-super font-sans text-base font-normal text-muted-foreground underline-offset-4 transition-colors hover:text-foreground hover:underline"
                    aria-label="Lihat sumber data angka ini"
                  >
                    1
                  </a>
                )}
        ...
        {/* Disclaimer — jangkar footnote [1] dari stats strip */}
        <m.p
          id="sumber-data"
          className="mt-10 max-w-prose scroll-mt-24 text-xs leading-relaxed text-muted-foreground sm:mt-12 sm:text-[13px]"
          initial={reduce ? false : { opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={inView}
