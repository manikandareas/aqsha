...
from api import extract_response, kickoff_research, poll_status

# ── Page config ─────────────────────────────────────────────

st.set_page_config(
    page_title="Deep Research Assistant",
    page_icon=str(Path(__file__).parent / "assets" / "crewai_logo.svg"),
    layout="wide",
...
LOGO_PATH = _HERE / "assets" / "crewai_logo.svg"
...
def _handle_crew_response(chat_id: str, prompt: str, status_area) -> str | None:
    kickoff_id = kickoff_research(prompt, chat_id, st.session_state.depth_mode)
    if not kickoff_id:
        return None
    ...
    for _ in range(max_attempts):
        ...
        task_text = html.escape(status.get("last_executed_task") or "Researching...")
    ...
    if prompt:
        ...
        try:
            with st.chat_message("assistant"):
                ...
                status_area.empty()
                if response_text == "__TIMEOUT__":
                    st.warning(
                        "Research is taking longer than expected. "
                        "The job may still be running — please try again shortly."
                    )
                elif response_text:
