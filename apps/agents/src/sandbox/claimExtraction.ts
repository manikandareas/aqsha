...
//
// Ported from the legacy Convex engine; the Vercel AI SDK `generateObject` call
// is replaced by the strict-JSON Messages API client in `llmExtract.ts` (the
// prompt gains an explicit output-shape section since schema steering is gone).
...
// rather than rejected at the schema layer (so one bad value does not fail the
// whole extraction).
...
export type RawExtraction = z.infer<typeof verificationExtractionSchema>;
...
const EXTRACTION_INSTRUCTION = [
  ...
  "",
  'Return a JSON object of this exact shape: {"grimClaims": [{"label": string, "mean": number, "sd": number|null, "n": number, "items": number|null}], "powerClaims": [{"label": string, "test": string, "n": number, "effectSize": number, "alpha": number|null, "tails": number|null}]}.',
].join("\n");
...
export async function extractVerificationClaims(
  ...
  if (!raw) return { grimClaims: [], powerClaims: [] };
  return normalizeExtractedClaims(raw);
}
