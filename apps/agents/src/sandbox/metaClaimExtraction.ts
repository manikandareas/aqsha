...
//
// Ported from the legacy Convex engine; `generateObject` is replaced by the
// strict-JSON Messages API client in `llmExtract.ts`.
...
export const metaExtractionSchema = z.object({ studies: z.array(studySchema) });

export type RawMetaExtraction = z.infer<typeof metaExtractionSchema>;
...
const EXTRACTION_INSTRUCTION = [
  ...
  "",
  'Return a JSON object of this exact shape: {"studies": [{"label": string, "yi": number|null, "vi": number|null, "sei": number|null, "mean1": number|null, "sd1": number|null, "n1": number|null, "mean2": number|null, "sd2": number|null, "n2": number|null, "events1": number|null, "events2": number|null}]}.',
].join("\n");

export async function extractMetaStudies(
  text: string,
  llm: JsonLlmClient,
  model: string,
): Promise<MetaStudy[]> {
  ...
  const raw = await llm.extractJson({
    ...
    schema: metaExtractionSchema,
  });
  if (!raw) return [];
  return normalizeMetaStudies(raw);
}
