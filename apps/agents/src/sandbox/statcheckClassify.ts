...
function reportedDecimals(value: number): number {
  ...
  if (s.includes("e") || s.includes("E")) {
    // (`?.[1]` rather than the legacy `m[1]`: this tsconfig enables
    // noUncheckedIndexedAccess; behavior is identical.)
    const exponent = /e-(\d+)/i.exec(s)?.[1];
    decimals = exponent ? parseInt(exponent, 10) : 0;
  } else {
    const dot = s.indexOf(".");
