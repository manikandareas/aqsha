...
// Mode hidup di URL (nuqs, satu query param `panel`) supaya tab Sitasi dan detail
...
export type WorkspacePanelMode =
  | { kind: "closed" }
  | { kind: "chat" }
  | { kind: "citations"; citationId?: string };
...
// nuqs parser: `null` (param absen / invalid) dibaca sebagai panel tertutup.
export const parseAsWorkspacePanelMode = createParser<WorkspacePanelMode>({
  parse(raw) {
    const mode = parseWorkspacePanelMode(raw);
    return mode.kind === "closed" ? null : mode;
  },
  serialize(mode) {
    return serializeWorkspacePanelMode(mode) ?? "";
  },
});
