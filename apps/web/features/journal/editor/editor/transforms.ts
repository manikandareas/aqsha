...
/**
 * Match PlaceholderPlugin.insert.media: stable `id` on the placeholder node plus
 * `withHistoryMark` when `disableEmptyPlaceholder` is on. Without an explicit
 * id, `updateUploadHistory` cannot match the insert_node op and crashes.
 */
const insertPlaceholderWithUploadHistory = (
  editor: PlateEditor,
  mediaType: string
) => {
  const runInsert = () => {
  ...
  if (disableEmptyPlaceholder) {
    editor.tf.withoutMerging(() => {
      withHistoryMark(editor, runInsert);
    });
  } else {
    runInsert();
  }
...
> = {
  ...
  [KEYS.audio]: (editor) =>
    insertPlaceholderWithUploadHistory(editor, KEYS.audio),
  ...
  [KEYS.file]: (editor) =>
    insertPlaceholderWithUploadHistory(editor, KEYS.file),
  [KEYS.img]: (editor) =>
    insertPlaceholderWithUploadHistory(editor, KEYS.img),
  ...
  [KEYS.video]: (editor) =>
    insertPlaceholderWithUploadHistory(editor, KEYS.video),
};
