...
import { ChevronRightIcon, FileTextIcon, FolderIcon, Loader2Icon } from "@aqsha/ui/icons";
import {
  ...
  return (
    <ComposerPopoverShell>
      ...
      <ComposerPopoverList id="composer-context-mentions">
        {capNotice ? (
          <p className="px-2 py-1.5 font-medium text-[11px] text-amber-600 dark:text-amber-400">
            {capNotice}
          </p>
        ) : null}
        {mode === "workspace" ? (
...
function DisabledReason({ reason }: { reason: string }) {
  return (
    <span className="max-w-[7.5rem] truncate text-[10.5px] text-muted-foreground">{reason}</span>
  );
}
