 ...
 *
 * Full mirror of `packages/ui/src/icons.tsx` (`@aqsha/ui/icons`): every
 * Aqsha-facing (Lucide-compatible) icon name maps to the SAME Hugeicons glyph
 * as web, so a name→glyph change in one app is a visible parity break in the
 * other (guarded by `icons.completeness.spec.ts`). App code (everything except
 * vendored `src/lib/components/ui/**`) imports icons from `$lib/icons`; direct
 * `@lucide/svelte` imports are banned by eslint.
 *
 ...
 *
 * This differs from web's React API (`<CheckIcon className="size-4" />`) because
 * `@hugeicons/svelte` renders via a single `HugeiconsIcon` that takes the glyph
 * as a prop — the same pattern shadcn-svelte vendored components already use.
 * Stroke/size defaults are inherited from `HugeiconsIcon` (unchanged vs web,
 * whose `createIcon` only normalized `strokeWidth`); size is driven by `size-*`
 * classes exactly as in web.
 *
 ...
 * mapping below is authoritative; keep it in lockstep with the React source.
 */
export { HugeiconsIcon as Icon } from '@hugeicons/svelte';
export type { HugeiconsProps, IconSvgElement } from '@hugeicons/svelte';

// Aqsha name → Hugeicons core-free glyph. Mirror of packages/ui/src/icons.tsx.
// Ordered/grouped to match the React `export const` block for review-by-diff.
export {
...
/**
 * Shared icon-per-artifact-type mapping (mirror of `getArtifactTypeIcon` in
 * packages/ui/src/icons.tsx). Returns the Hugeicons glyph; render with
 * `<Icon icon={getArtifactTypeIcon(type)} />`. The React `ArtifactTypeIcon`
 * component wrapper is deferred to the artifact surfaces (Phase 9).
 */
