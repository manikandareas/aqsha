...
/**
 * Per-run replay/idempotency filter. The single long-lived `subscribeToThread` replays the ENTIRE buffer
 * of an in-flight run from index 0 on every (re)connect (refresh, network blip, or lite→pro channel flip
 * mid-run). `text-delta` reduction APPENDS (non-idempotent), so a replayed chunk must be dropped before
 * it reaches the reducer. This is the guarantee behind "no duplicate / no lost tokens".
 *
