...
// @aqsha/services). apps/svelte tidak meng-import services — tipe dicermin manual
...
export type CitationCandidate = CitationListItem & { linked: boolean };
...
export function citationMetaLine(item: {
	...
	const head = [first ? `${first}${extra}` : null, item.publishedYear].filter(Boolean).join(' · ');
