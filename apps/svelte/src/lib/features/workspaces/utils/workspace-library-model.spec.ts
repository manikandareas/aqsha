...
describe('workspace library model', () => {
	it('groups active artifacts into root and one-level folders', () => {
		...
		expect(groups[1]?.artifacts.map((artifact) => artifact._id)).toEqual(['artifact-folder']);
	});
		...
		expect(
			expectArtifactsReturnToRootAfterFolderDelete({
				...
				after: artifacts.map((artifact) =>
					artifact.folderId === 'folder-a' ? { ...artifact, folderId: undefined } : artifact
				),
	...
	it('separates global and workspace-bound sidebar threads', () => {
		...
		const global = items.filter((thread) => !('workspaceId' in thread && thread.workspaceId));
		const workspaceBound = items.filter(
			(thread) => 'workspaceId' in thread && Boolean(thread.workspaceId)
		);
