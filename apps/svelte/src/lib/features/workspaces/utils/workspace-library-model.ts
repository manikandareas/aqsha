...
function sortWorkspaceArtifacts(artifacts: WorkspaceArtifact[], sort: WorkspaceArtifactSort) {
...
export function getMoveTargetOptions(folders: WorkspaceFolder[]): MoveTargetOption[] {
	const activeFolders = folders.flatMap((folder) => (folder.status === 'deleted' ? [] : [folder]));
...
export function getWorkspaceMoveTargetOptions(
	...
	return workspaces.flatMap((workspace) =>
		workspace._id === currentWorkspaceId ? [] : [{ value: workspace._id, label: workspace.name }]
	);
}
...
export function resolveActiveFolderId({
	...
	const exists = groups.some((group) => group.folder !== null && group.id === activeFolderId);
...
export function expectArtifactsReturnToRootAfterFolderDelete({
	...
	const movedIds = new Set(
		before.flatMap((artifact) =>
			artifact.folderId === deletedFolderId && artifact.status !== 'deleted' ? [artifact._id] : []
		)
	);
