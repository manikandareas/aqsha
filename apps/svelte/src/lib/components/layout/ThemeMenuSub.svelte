...
<DropdownMenu.Sub>
	<DropdownMenu.SubTrigger
		class={cn(compactMenuItemClass, 'w-full cursor-default [&>svg:last-child]:hidden', className)}
	>
	...
	<DropdownMenu.SubContent class="min-w-36 rounded-[12px] p-1.5">
		{#each themeOptions as option (option.key)}
			...
			<DropdownMenu.Item class={compactMenuItemClass} onSelect={() => theme.setMode(option.key)}>
