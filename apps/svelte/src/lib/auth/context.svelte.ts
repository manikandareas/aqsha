...
/**
 * Auth facade sisi CLIENT. Feature code memakai facade ini, BUKAN `svelte-clerk` mentah — supaya
 * API package auth tak tersebar dan gotcha berikut ter-bake:
 *
 *  - Gate di `isLoaded`, bukan `userId`: `userId` datang instan dari SSR `initialState`, tapi
 *    `session.getToken()` masih `null` sampai clerk-js load → firing lebih awal = 401 tokenless.
 ...
 *    membaca objek `clerk.auth` langsung di `$effect` bikin re-run tiap churn token → state
 *    ke-destroy mid-stream.
 *
 ...
 * token segar (bukan captured/stale). Sisi server: `getServerToken` di `$lib/server/auth`.
 */
 ...
 * lanjutan (settings/security) tanpa menyebar `useClerkContext` ke feature. Reaktif: baca
...
/**
 * Reactive `{ isLoaded, user }` facade. The `user` is the clerk-js `UserResource`
 * (`passwordEnabled`/`totpEnabled`/`updatePassword`/`createTOTP`/…) used by security panels.
 * Getters re-run as clerk-js loads/refreshes the user.
 */
 ...
 * reverification flow. Captures the clerk context here (init
