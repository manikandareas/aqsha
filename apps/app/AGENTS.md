...
|---|---|---|---|---|
| Thread detail | `/threads/[threadId]` | `features/thread-experience/components/thread-detail-shell.tsx` (`ThreadDetailShell`) | `ChatThreadState` via `thread-shell-layout.tsx` | `ThreadWorkspaceLibraryPanel` → `WorkspaceLibrarySurface`, or `ThreadGlobalContextPanel` |
| Workspace detail | `/workspaces/[workspaceId]` | `app/workspaces/[workspaceId]/workspace-detail-client.tsx` (`WorkspaceDetailClient`) | `WorkspaceLibrarySurface` | `features/workspaces/components/workspace-chat-side-panel.tsx` (`WorkspaceChatSidePanel`) |
...
Product copy and right-panel intent also live in `apps/app/DESIGN.md` (Right Panel section).
