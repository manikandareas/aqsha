      ...
      <SidebarMenuItem>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <SidebarMenuButton
              size="lg"
              className="h-10 min-w-0 rounded-[8px] border border-sidebar-border/70 bg-muted/15 px-2.5 py-2.5 text-muted-foreground hover:bg-muted/45 hover:text-foreground data-[state=open]:bg-muted/55 data-[state=open]:text-foreground"
            >
              <Avatar className="h-6 w-6 shrink-0 rounded-full ring-1 ring-[var(--sky-soft-border)]">
                {user?.image ? <AvatarImage src={user.image} alt={name} /> : null}
                <AvatarFallback className="rounded-full bg-[var(--sky-soft)] text-[10px] font-semibold text-primary">
                  {initials}
                </AvatarFallback>
              </Avatar>
                  ...
                  <DropdownMenuItem key={workspace._id} asChild className={menuItemClass}>
                    <Link href={`/workspaces/${workspace._id}`}>
                      ...
                      <span
                        className={cn(
                          "truncate",
                          workspace._id === activeWorkspaceId && "font-semibold",
                        )}
                      >
                        {workspace.name}
                      </span>
