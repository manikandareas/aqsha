...
import { useResearchPanelState } from "./research-panel-state";
import type {
  ChatMessage,
  RateStatus,
  ResearchArtifact,
...
export function ThreadExperience({ threadId }: { threadId?: string }) {
  ...
  const panelState = useResearchPanelState({
    hasSources: Boolean(sources && sources.length > 0),
    artifacts,
    runs: runs ?? [],
  });
  const activeArtifact = useQuery(
    api.agent.artifacts.get,
    isAuthenticated && panelState.selectedArtifactId
      ? { artifactId: panelState.selectedArtifactId as never }
      : "skip",
